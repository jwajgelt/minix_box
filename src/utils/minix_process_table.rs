...
impl MinixProcessTable {
    ...
    pub fn get(self: &Self, endpoint: Endpoint) -> Option<&MinixProcess> {
        let idx = endpoint as usize;
        self.table[idx].as_ref()
    }

    pub fn get_mut(self: &mut Self, endpoint: Endpoint) -> Option<&mut MinixProcess> {
        let idx = endpoint as usize;
    ...
    pub fn get_by_pid(self: &Self, pid: Pid) -> Option<&MinixProcess> {
    ...
    /// with the given (Linux) pid
    pub fn get_mut_by_pid(self: &mut Self, pid: Pid) -> Option<&mut MinixProcess> {
        let idx = *self.pid_map.get(&pid)?;
        self.table[idx].as_mut()
    }

    pub fn insert(self: &mut Self, proc: MinixProcess, endpoint: Endpoint) -> Result<(), ()> {
        let idx = endpoint as usize;
    ...
    pub fn remove(self: &mut Self, endpoint: Endpoint) -> Option<MinixProcess> {
        let idx = endpoint as usize;
